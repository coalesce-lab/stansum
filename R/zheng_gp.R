...
#' @rdname zheng_gp
#'
#' @description - `zheng_gp_count` -- model for count responses
#'
...
#'
#' @export
zheng_gp_count <- function(N, K, y, ...) {
  mod <- get_model("ZhengGP_count")
  d <- list(N=N, K=K, y=y)
  mod$sample(data = d, ...)
}
...
#' @rdname zheng_gp
#'
#' @description - `zheng_gp_dichotomous` -- model for dichotmous responses
#'   proposed by Baum & Marsden (2023).
#'
#' @export
zheng_gp_dichotomous <- function(N, K, y, ...) {
  mod <- get_model("ZhengGP_dichotomous")
  d <- list(N=N, K=K, y=y)
  mod$sample(data = d, ...)
}
